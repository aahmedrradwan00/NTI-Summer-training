...
export class OrdersService {
    ...
    getOrder(orderId: string): Observable<any> {
        return this._HttpClient.get(`${this.apiUrl}${this.routeName}/${orderId}`, { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } });
    }

    updateDeliveredOrder(orderId: string): Observable<any> {
        return this._HttpClient.put(`${this.apiUrl}${this.routeName}/${orderId}/delivered`, {}, { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } });
    }

    updatePaidOrder(orderId: string): Observable<any> {
        return this._HttpClient.put(`${this.apiUrl}${this.routeName}/${orderId}/paid`, {}, { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } });
    }
}
