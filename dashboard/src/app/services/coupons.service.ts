...
export class CouponsService {
    ...
    getCoupon(couponId: string): Observable<any> {
        return this._HttpClient.get(`${this.apiUrl}${this.routeName}/${couponId}`, { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } });
    }
    ...
    updateCoupon(couponId: string, formData: any): Observable<any> {
        return this._HttpClient.put(`${this.apiUrl}${this.routeName}/${couponId}`, formData, { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } });
    }

    deleteCoupon(couponId: string): Observable<any> {
        return this._HttpClient.delete(`${this.apiUrl}${this.routeName}/${couponId}`, { headers: { authorization: `Bearer ${localStorage.getItem('user')}` } });
    }
}
