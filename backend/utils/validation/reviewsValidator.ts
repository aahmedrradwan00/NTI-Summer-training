...
export const createReviewValidator: RequestHandler[] = [
    ...
    check('user').notEmpty().withMessage('user Required').isMongoId().withMessage('invalid Mongo id'),
    check('product')
        .notEmpty()
...
export const updateReviewValidator: RequestHandler[] = [
    check('id')
        ...
        .custom(async (val, { req }) => {
            ...
            if (review.user._id!.toString() !== req.user._id.toString()) throw new Error('you are not allowed to perform this action');
            return true;
        }),
    validatorMiddleware,
];

export const getReviewValidator: RequestHandler[] = [check('id').isMongoId().withMessage('Invalid Mongo Id'), validatorMiddleware];
