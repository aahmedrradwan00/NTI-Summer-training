...
class Features {
    ...
    filter() {
        ...
        excludedFields.forEach((el: string): void => {
    ...
    search(modelName: string) {
        if (this.queryString.search) {
            ...
            if (modelName === 'products') {
                query.$or = [{ name: new RegExp(this.queryString.search, 'i') }, { description: new RegExp(this.queryString.search, 'i') }];
            } else query = { name: new RegExp(this.queryString.search, 'i') };
            this.mongooseQuery = this.mongooseQuery.find(query);
        }
        return this;
    }
    pageination(documentsCount: number) {
        ...
        if (endIndex < documentsCount) pagination.next = Number(page) + 1;
